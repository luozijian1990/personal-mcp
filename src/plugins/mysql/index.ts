...
export function createMysqlPlugin(options: { config?: MysqlPluginConfig; executor?: MysqlExecutor } = {}): PersonalMcpPlugin {
  ...
  return { ...MYSQL_PLUGIN_METADATA, tools: [{ name: "execute_sql", title: "执行 SQL", risk: "privileged", requiresConfirmation: true, logging: { input: "redacted", output: "none" } }], config: { fields: MYSQL_CONFIG_FIELDS }, checkHealth: async (signal) => { validateMysqlConfig(config); const conn = await createConnection({ host: config.host, port: config.port, user: config.user, password: config.password, database: config.database, connectTimeout: 5_000 }); const abort = () => (conn as unknown as { destroy: () => void }).destroy(); signal.addEventListener("abort", abort, { once: true }); try { await conn.query("SELECT 1"); return { state: "healthy", message: "MySQL connection is healthy" }; } finally { signal.removeEventListener("abort", abort); await conn.end().catch(() => undefined); } }, createServer: () => createMysqlServer(config, executor) };
}
